class SlideshowsController < ApplicationController
  ...
  def create
    ...
    selected_images_string = params[:selected_images]
    selected_images_array = selected_images_string.split('|')

    respond_to do |format|
      if @slideshow.save

        selected_images_array.each do |image_id|
          PhotoArray.create(slideshow_id: @slideshow.id, photograph_id: image_id)
        end
